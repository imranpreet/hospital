...
// Admin registration with server-side passkey validation
router.post('/admin-register', async (req, res) => {
  try {
    const { name, email, password, passkey, contact } = req.body;
    if (!name || !email || !password || !passkey) {
      return res.status(400).json({ msg: 'Name, email, password, and admin passkey are required' });
    }
    ...
    const hashed = await bcrypt.hash(password, 10);
    const user = new User({ name, email, password: hashed, role: 'admin', contact });
    await user.save();
